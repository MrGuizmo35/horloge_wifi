...
int32_t DRV_SPI0_ISRSlaveEBM8BitTasks ( struct DRV_SPI_OBJ * dObj )
{
    ...
    do {
        ...
        if (dObj->currentJob == NULL)
        {
            ...
            if (currentJob->dataLeftToTx > PLIB_SPI_RX_8BIT_FIFO_SIZE(SPI_ID_1))
            {
        ...
        {
            DRV_SPI0_SlaveEBMReceive8BitISR(dObj);
        }
        if (currentJob->dataLeftToTx != 0)
        {
            DRV_SPI0_SlaveEBMSend8BitISR(dObj);
        }
        else
        {
            dObj->txEnabled = false;
        ...
        if (dObj->currentJob != NULL)
        {
            ...
            if ((SYS_INT_SourceStatusGet(INT_SOURCE_SPI_1_RECEIVE)) || (SYS_INT_SourceStatusGet(INT_SOURCE_SPI_1_ERROR)))
            {
